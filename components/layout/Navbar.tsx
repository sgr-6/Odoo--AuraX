...
export function Navbar() {
  ...
  // Safety check since GlobalStore might not be wrapped yet during hot reloads
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Avatar className="h-10 w-10 border border-gray-200 cursor-pointer hover:border-indigo-600 transition-all duration-200">
              ...
              <AvatarFallback className="bg-indigo-600 text-white font-bold text-lg flex items-center justify-center">
                {currentUser ? (store.employees.find(e => e.id === currentUser.empId)?.name?.charAt(0) || currentUser.email.charAt(0).toUpperCase()) : 'E'}
              </AvatarFallback>
            </Avatar>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56 bg-white border border-gray-200 rounded-xl shadow-lg mt-1 p-2">
            <DropdownMenuLabel className="font-bold text-gray-900 px-2 py-1.5">
              {currentUser ? (store.employees.find(e => e.id === currentUser.empId)?.name || currentUser.email) : 'Admin User'}
              <div className="text-xs font-normal text-gray-500 mt-0.5">{currentUser?.email || 'admin@dayflow.demo'}</div>
            </DropdownMenuLabel>
